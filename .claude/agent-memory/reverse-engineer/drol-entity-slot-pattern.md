...
| enemy-B | $E0   | $E1/$E2    | $E3  | $E4       | $0231     | SFX_TONE        | $1536 ENEMY_B_ADVANCE   | $1591 SPRITE_DRAW_3  |
| enemy-C | $D4   | $D5/$D6    | $D7  | $B7       | $0224     | SND_DELAY_DOWN  | $1646 ENEMY_C_ADVANCE   | $16AC PLAYER_RENDER  |

Main-loop order: ENEMY_C_ADVANCE, PLAYER_RENDER, ENEMY_A_ADVANCE, SPRITE_DRAW_1, ENEMY_B_ADVANCE, SPRITE_DRAW_3, BEAM_UPDATE.
...
**Cross-slot handshake topology (3-slot, confirmed):**
- A's rearm at $1411 writes $01 to $D4 (wakes C in steady mode).
- C's rearm writes $00 to $D4 (deactivates self); no outgoing handshake.
- B's rearm writes neither $DC nor $D4; the B->{A,C} firing handshake lives in the **draw** routine SPRITE_DRAW_3 at $1757-$1763 (writes $FF to $D4 always, $FF to $DC if $DC != 0 --- puts both A and C into drift mode).
...
**Why:** Recognizing this triplet structure turned a 976-byte opaque HEX blob ($13D6-$17A5) into six documentable routines, then carved ENEMY_B_ADVANCE out of the middle of SPRITE_DRAW_1. The triplet is now fully decomposed on the tick side; the three draw routines at $1423/$1591/$16AC remain HEX blobs sharing a parallel structure.

**How to apply:** When RE'ing the three draw routines, expect: (1) early `LDA $Xx; BEQ exit` state gate; (2) clip-against-player-column logic using $4A/$1A8F; (3) sprite parameter setup ($56/$57/$5B/$5D) + SMC patches at $65B3/$65B4 + JSR DRAW_SPRITE ($656F); (4) for SPRITE_DRAW_1/3 a hit-detection block writing to the cross-slot state bytes ($DC/$D4); (5) the slot-deactivation code that zeros its own state.
